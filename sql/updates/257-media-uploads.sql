...
ALTER TABLE content ADD COLUMN image_id UUID REFERENCES image(image_id);
CREATE INDEX idx_content_image_id ON content(image_id) WHERE image_id IS NOT NULL;

UPDATE content c
SET image_id=i.image_id
FROM image i
WHERE c.image_file_upload_id=i.file_upload_id
...
AS SELECT c.content_id,
    ...
    inst.name AS owner_institution,
    c.date_created,
    c.file_upload_id,
    c.image_id,
    ...
    COALESCE(i.url, fu2.url) as image_url,
    fu.filesize
   FROM content_type ct,
    ...
    LEFT OUTER JOIN v_image i ON c.image_id = i.image_id
  WHERE c.content_type_id::text = ct.content_type_id::text
  AND c.owner_institution_id::text = inst.institution_id::text
