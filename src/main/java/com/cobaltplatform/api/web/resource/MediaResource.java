...
public class MediaResource {
	...
	@Nonnull
	private final MediaImageUploadResultApiResponseFactory mediaImageUploadResultApiResponseFactory;
	@Nonnull
	...
	@Nonnull
	@POST("/media/images/presigned-upload")
	@AuthenticationRequired
	public ApiResponse createMediaImagePresignedUpload(@Nonnull @RequestBody String requestBody) {
		requireNonNull(requestBody);

		Account account = getCurrentContext().getAccount().get();
		CreateMediaImagePresignedUploadRequest request = getRequestBodyParser().parse(requestBody, CreateMediaImagePresignedUploadRequest.class);
		MediaImageUploadResult mediaImageUploadResult = getMediaService().createMediaImagePresignedUpload(account, request);

		return new ApiResponse(new HashMap<String, Object>() {{
			put("mediaImageUploadResult", getMediaImageUploadResultApiResponseFactory().create(mediaImageUploadResult));
		}});
	}

	@Nonnull
	@POST("/media/images/{imageId}/uploaded")
	@AuthenticationRequired
	public ApiResponse confirmMediaImageUploaded(@Nonnull @PathParameter UUID imageId) {
		requireNonNull(imageId);

		Account account = getCurrentContext().getAccount().get();
		Image image = getMediaService().confirmMediaImageUploaded(account, imageId);

		return new ApiResponse(new HashMap<String, Object>() {{
			put("image", getMediaImageApiResponseFactory().create(image));
		}});
	}
	...
	@Nonnull
	protected MediaImageUploadResultApiResponseFactory getMediaImageUploadResultApiResponseFactory() {
		return this.mediaImageUploadResultApiResponseFactory;
	}

	@Nonnull
	protected MediaImageApiResponseFactory getMediaImageApiResponseFactory() {
		return this.mediaImageApiResponseFactory;
	}
